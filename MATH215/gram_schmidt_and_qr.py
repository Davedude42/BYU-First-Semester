...
def QR_decomposition(A):
  Q_cols = gram_schmidt([np.array(A[:,j]) for j in range(len(A[0]))])
  Q = np.zeros([len(Q_cols[0]), len(Q_cols)])
  for i in range(len(Q_cols[0])):
...
def QR_solver(A,b):
  Q, R = QR_decomposition(A)
  return back_substitution(R, np.transpose(Q) @ b)
