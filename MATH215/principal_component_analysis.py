...
def projection_2D(u1,u2,X):
  U = np.array([np.transpose(u1),np.transpose(u2)])
  return U @ X
...
W= 1 / (X_total.shape[1]-1) * (X_total @ np.transpose(X_total))
...
X_proj = projection_2D(u1, u2, X_total)

# Calculate the covariance matrix of the projected data
W_proj = 1 / (X_proj.shape[1] - 1) * (X_proj @ np.transpose(X_proj))

# Find the eigenvalues of the projected covariance matrix
L_proj, _ = np.linalg.eig(W_proj)

# Sum the eigenvalues to get the reduced variance
reduced_variance = np.sum(L_proj)
...
Alice_2D=projection_2D(u1, u2, np.transpose(Alice))

Bob_2D=projection_2D(u1, u2, np.transpose(Bob))
...
plot_data(X_neg_2D, X_pos_2D, Bob_2D)
