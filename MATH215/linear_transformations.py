...
def transform(x):
  # Put your code here.
  return np.array([[2,1], [1,-3], [0,1]]) @ np.array(x)
...
def reflect(image,a,b):
  T = 1/(a**2 + b**2) * np.array([[a**2 - b**2, 2*a*b], [2*a*b, b**2 - a**2]])
  return T @ np.array(image)
...
def area(A):
  # Put your code here.
  return np.linalg.norm(np.cross(np.array([A[0][0], A[1][0], 0]), np.array([A[0][1], A[1][1], 0])))
